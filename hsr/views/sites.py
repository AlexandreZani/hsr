...
def sites(pipe, environ, start_response):
  ...
  sites_q = session.query(Site).order_by(sort_column).offset(offset).limit(limit)
  sites = sites_q.all()

  status = "200 OK"
  template = environ['pythia']['jinja_env'].get_template("hsr/sites.html")
  data = template.render(context, sites=sites)
  response_headers = [
